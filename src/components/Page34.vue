  ...
  export default {
    ...
    data () {
      return {
        ...
        popups: [
          {
            html: '<h1>Usar a persona TIM<small>Uma espécie de identidade padrão</small></h1><h4>A comunicação com o cliente devem sempre ser:</h4><span>coesas e padronizadas</span><span>Cada produto pode ter uma linguagem diferente</span><h4>PERSONA TIM é:</h4><span>Carismática</span><span>leve</span><span>autêntica e espontânea</span>'
          }, {
            html: '<h1>Comunicar de forma clara e transparente<small>Informações transparentes, completas e diretas em todos os momentos de seu relacionamento com a TIM</small></h1><h4>Usar os 7 Cs da Comunicação:</h4><span>Claro</span><span>Conciso</span><span>Concreto</span><span>Correto</span><span>Coerente</span><span>Completo</span><span>Cortês</span><h4>Para esclarecer, usar técnicas como:</h4><span>Perguntar</span><span>repetir</span><span>sumarizar</span><span>exemplificar</span>'
          }, {
            html: '<h1>Falar a língua do cliente<small>As mensagens enviadas precisam conter a LINGUAGEM DO CLIENTE, não a linguagem da operadora</small></h1><h4>Criar empatia com o cliente</h4><span>crie conexão</span><span>seja objetivo</span><h4>Sempre lembrar:</h4><span>Falar diferente com gente diferente</span><span>Considerar a linguagem do cliente</span><span>Dispensar o termo técnico.</span>'
          }, {
            html: '<h1>Se mudar, tem que comunicar<small>Sempre que um serviço mudar – seja seu preço ou condições – o cliente precisa ser informado imediatamente.</small></h1><h4>Ao comunicar uma mudança é importante:</h4><span>pensar além do cumprimento de regras</span><span>Considerar como essas mudanças afetarão o cliente</span>'
          }, {
            html: '<h1>Encantar o cliente.<small>Por que o cliente escolheu comprar nosso produto? Qual foi o desejo dele?</small></h1><h4>Lembrando que:</h4><span>cada pessoa pode escolher o mesmo produto por diferentes razões</span><h4>Adequar a comunicação:</h4><span>ofereceça o que realmente é do interesse dele!</span>'
          }
        ]
      }
    },
